<?
  ...
  $qry_contrato = farray(query("select * from is_contrato where numreg = '$pnumreg'"));
  $qry_empresa = farray(query("select * from is_pessoa where numreg = '".$qry_contrato["id_pessoa"]."'"));
  $qry_pessoa = farray(query("select * from is_contato where numreg = '".$qry_contrato["id_pessoa_contato"]."'"));
  $qry_usuario = farray(query("select * from is_usuario where numreg = '".$qry_contrato["id_usuario_resp"]."'"));

  $qry_modelo = farray(query("select id_arquivo from is_tp_contrato where numreg = '".$qry_contrato["id_tp_contrato"]."'"));
  ...
  $sql_produtos = (query("select * from is_contrato_obj where id_contrato = '".$qry_contrato["id_contrato"]."'"));
  ...
  $texto=(str_replace("VS_NUMERO_CONTRATO",utf8_encode($qry_contrato["nr_contrato"]),$texto));
  ...
  $texto=(str_replace("VS_CONTRATO_OBS",utf8_encode($qry_contrato["obs"]),$texto));
