 ...
 * 9 - Clonar
 * 10 - Criar pedido Bonificação
 * 11 - Enviar por E-mail
 * 12 - Excluir Item
 * 13 - Finalizar
 * 14 - Impressão
 * 15 - Reabrir Orçamento
 * 16 - Reprovação
 * 17 - Restaurar Revisão
 * 18 - Transformar em Pedido
 * 19 - Alterar Comissão do Item
 * 20 - Atualização Atendimento Laboratório
 * 21 - Log Exportação ODBC
 * 
 * Origens do Sistema
...
class Venda{
    ...
    public $Debug = true; //Define se o modo de Debug ser? ativo
    ...
    public function CancelaVenda($IdMotivoCancelamento){
        if($this->getDadosVenda('id_situacao_venda') <= 1){
            ...
            if($this->AtualizaDadosVendaBD()){
                $this->GravaLogBD(6,'Cancelamento da Venda');
                return true;
            }
        }
        else{
    ...
    public function getTitleBar(){
        ...
        $TituloVenda = $this->TituloVenda;
        $TituloVenda = ($this->isTipoBonificacao())?$TituloVenda.' de bonifica??o':$TituloVenda;
        ...
        $StringRetorno .= '<span class="venda_span_header">Voc&ecirc; est&aacute; no '.$StringPassoAtual.' ('.$StringDetalhePassoAtual.') <span>N&ordm; '.$this->getNumregVenda().'</span></span>';
    ...
    public function getItemPorIdProduto($IdProduto){
        ...
        foreach($Itens as $Item){
            if($Item->getNumregProduto() == $IdProduto){
                return $Item;
            }
        }
        return false;
    ...
    public function getArTabPrecos(){
        ...
        $SqlTabPreco = "SELECT numreg,nome_tab_preco FROM is_tab_preco WHERE sn_ativa = 1";
        ...
        if($NumRowsTabPreco == 0){
            ...
            while($ArTabPrecos = farray($QryTabPreco)){
                $this->ArTabPrecos[] = array($ArTabPrecos['numreg'],$ArTabPrecos['nome_tab_preco']);
            }
        }
        return $this->ArTabPrecos;
    }
    ...
    public function PreencheIdVendaClienteBD(){
        ...
        $ArUpdateVenda = array();
        $ArUpdateVenda['numreg']                = $this->getNumregVenda();
        $ArUpdateVenda['id_venda_cliente']      = getParametrosVenda('prefixo_numero_pedido').$MaxId;
    ...
    public function GeraClone(){
        ...
        if(!$NovaVenda->RecalculaVenda()){
            $this->setMensagem('N?o foi poss?vel atualizar os dados do or?amento.');
