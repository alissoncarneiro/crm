...
$id_agenda= $_REQUEST['id_agenda'];
	...
	$sqlAgendaLote = "select id_pessoa from c_coaching_inscricao_curso_detalhe where id_agenda = $id_agenda and sn_realocada = 0 group by id_pessoa limit 1";

	$qryAgendaLote = query($sqlAgendaLote);
		...
		$SqlCursoEmailPessoa = "
			SELECT
				  ...
				  FROM c_coaching_inscricao_curso_detalhe AS InsCursoDetalhe
					...
					$innerParte
				  WHERE
					InsCursoDetalhe.id_pessoa = '".$arAgendaLote['id_pessoa']."' AND  modeloPeca.numreg = '".$modelo."' $whereParte
