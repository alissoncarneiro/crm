<?php
/*
 * class.PoliticaComercialDescVendaMedia.php
 * Autor: Alex
 * 08/12/2010 14:18
 * - Classe para tratar a política comercial da venda, baseada na média de desconto total da venda.
 *
 * Log de Alterações
 * yyyy-mm-dd <Pessoa responsável> <Descrição das alterações>
 */

class PoliticaComercialDescVendaMedia{
    ...
    public function ValidaPolitica($PctMedDesc){
        ...
        $SqlPoliticaVenda = "SELECT ".((TipoBancoDados == 'mssql')?'TOP(1)':'')." pct_max_desc FROM is_param_politica_comercial_desc_venda_media WHERE sn_ativo = 1 AND ('".date("Y-m-d")."' BETWEEN dthr_validade_ini AND dthr_validade_fim)";
        ...
        if($ArPoliticaVenda['pct_max_desc'] < $PctMedDesc){
            $this->Status = false;
            $this->setStringStatus('fora da pol&iacute;tica comercial. M&eacute;dia de desconto: '.number_format($PctMedDesc,2,',','').'%');
            return false;
        }
        else{
            $this->Status = true;
            $this->setStringStatus('OK. M&eacute;dia de desconto: '.number_format($PctMedDesc,2,',','').'%');
            return true;
        }
    }
}
