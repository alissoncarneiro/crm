...
foreach($Venda->getItens() as $IndiceItem => $Item){
    if($_POST['controle_alteracao_'.$Item->getNumregItem()] != 1){//Se o produto não foi alterado, não processa-o
        continue;
    }
    $Item->setDadosItemPOST($_POST);
    $AtualizaItem = $Item->AtualizaItemBD();
    if($AtualizaItem[0] !== true){//Se houve algum erro, informa ao usuário
        $Erro = true;
    }
}
