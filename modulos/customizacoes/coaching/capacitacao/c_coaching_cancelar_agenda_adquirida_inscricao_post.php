...
$Inscricao = $_POST['id_inscricao'];

$SqlInscricaoCurso = "select numreg, id_agenda from c_coaching_inscricao_curso WHERE id_inscricao = '$Inscricao' and id_situacao <> 4";
$QrySqlInscricaoCurso = query($SqlInscricaoCurso);
while($ArQrySqlInscricaoCurso = farray($QrySqlInscricaoCurso)){
	$ArUpdate = array(
		'numreg'                    => $ArQrySqlInscricaoCurso['numreg'],
		'id_situacao'               => '3',
	...
	if(query($SqlArUpdate)){
		...
		$SqlDeleteAgendas = "DELETE FROM c_coaching_inscricao_curso_detalhe WHERE 
												id_inscricao = '".$Inscricao."'  AND 
													id_agenda = '".$ArQrySqlInscricaoCurso['id_agenda']."'";
		
		query($SqlDeleteAgendas);
		$sqlUpdateSituacao = "UPDATE c_coaching_inscricao SET id_situacao = '5' WHERE NUMREG = '".$Inscricao."'";
		if(query($sqlUpdateSituacao)){
			$Status = 1;
		...
		if($_POST['edtc_id_tipo_desistencia'] == '1'){
			...
			$ArInsertAtividade = array(
				...
				'obs'               => 'Curso: '.$ArAgendaCurso['nome_curso']."\r\n".'Valor Devolṳ̣o: '.number_format(TrataFloatPost($_POST['edtc_vl_devolucao']),2,',','.')
			);
