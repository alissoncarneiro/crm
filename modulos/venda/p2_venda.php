...
/*
 * Carregando os itens da solicitação comercial quando houver
 */
...
<fieldset><legend>Adicionar Itens</legend>
    <?php if(!$Venda->getSnGeradoBonificacaoAuto()){ /* Se a venda não foi gerada automaticamente a partir de outro pedido */?>
        ...
        <input type="button" class="botao_jquery venda_btn_add_produto_familia" value="+ Incluir Produto Por Família">
        <script>
        $(document).ready(function(){
        ...
        <input type="button" class="botao_jquery venda_btn_multiplo_orc" value="Unificar Orçamentos">
        <script>
        $(document).ready(function(){
        ...
        <?php } ?>
        <?php } else { /* FIM Se a venda não foi gerada automaticamente a partir de outro pedido */
    ...
    $(document).ready(function(){
        ...
        $("#btn_proximo_passo").click(function(e){
            if($("#controle_item_alterado").val() == 1){
                ...
                $("#jquery-dialog").html('Há itens pendentes de atualização. Ao passar para o próximo passo as alterações não serão gravadas. Deseja continuar ?');
                $("#jquery-dialog").dialog({
