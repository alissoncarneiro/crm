<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
	...
	function inseriHtml(ordem){	
		modelo_ordem = ordem;
		$.post('modulos/customizacoes/coaching/email_pessoa/retorno_peca_lote.php',                          
		({
			acao: ordem,
			id_agenda:<?=$_REQUEST['numreg']?>
		}),                              												
		function(retorno){                                       								
			if (retorno !='') {																	                           
    ...
    if($_SESSION['id_perfil'] != '4'){
	    ...
	    $sql_modelo = "select * from is_modelo_orcamento where sn_envia_email=1 and numreg not in(5,6,7)";
	}
    $qry_modelo = query($sql_modelo);
    ?>
    <div id="ret"> </div>
    <form action="modulos/customizacoes/coaching/email_pessoa/agenda_email_lote.php" id="form_envio_email" name="form" method="post" enctype="multipart/form-data" onsubmit="return AIM.submit(this, {'onStart' : UploadstartCallback, 'onComplete' : UploadcompleteCallback});">
    <table border="0" cellpadding="5" cellspacing="0">
        <tr>
          <td>Modelo do envio</td>
          <td><select id="select_id_modelo" name="select_id_modelo"  onChange="inseriHtml(this.value);">
        ...
        <tr>
            ...
            <td colspan="2"><textarea name="text_area_corpo_email" id="text_area_corpo_email" class="tinymce" style="width:100%; height:300px;"></textarea>
            </td>
        </tr>
        <tr>
    ...
    <fieldset>
     ...
     <input type="submit" value="Enviar E-mail" name="submit" id="btn_eviar" style="margin-top: 15px; padding: 5px;" />
    <input type="hidden" name="pnumreg" id="pnumreg" value="<?php echo $_REQUEST['numreg'];?>" />
    </form>
