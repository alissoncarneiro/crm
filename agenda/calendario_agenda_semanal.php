...
$qty_dias_semana = (!empty($_GET['dias_semana'])) ? $_GET['dias_semana'] : 5;
$dias_desconto = ($qty_dias_semana == 7) ? 1 : 2;

if (date("D", strtotime($dt_inicio) != 2)) {
    $dt_inicio = date("Y-m-d", strtotime($dt_inicio . " - " . (($semana_n[date("D", strtotime($dt_inicio))] - $dias_desconto) . " days")));
}
...
if ($ar_perfil['sn_bloquear_leitura'] == 'N') {
...
for ($i = $hr_inicio_n; $i <= $hr_fim_n; $i++) {
    echo '<div class="cal_time" style="z-index:1;top:' . $height . 'px;left:' . $base_left . 'px;">' . (($i < 10) ? '0' : '') . $i . '<span class="cal_time_font_small">00</span></div>';
    $height = $height + 60;
    $loop_hr = $loop_hr + 1;
}
...
if (count($ar_ativ_back_exibidas) > 0) {
    $style_div_back = 'background:#A5BFE1;position:absolute;top:' . ($base_height_atual - (20 * count($ar_ativ_back_exibidas))) . 'px;left:' . $left . 'px;width:' . ($size_width * $dias) . 'px;height:' . (20 * count($ar_ativ_back_exibidas) - 2) . 'px;border:1px solid #000000;';
    ...
    for ($w = 0; $w < count($ar_ativ_back_exibidas); $w++) {
        ...
        if ($qry_atividade['dt_inicio'] <= $dt_inicio_loop) {
            ...
            if ($qry_atividade['dt_prev_fim'] >= $dt_ultimo_dia) {
                $width_div = $size_width * (diferenca_dt($dt_inicio_loop, $dt_ultimo_dia) + 1);
            } else {
                $width_div = $size_width * (diferenca_dt($dt_inicio_loop, $qry_atividade['dt_prev_fim']) + 1);
            }
        }
        ...
        elseif ($qry_atividade['dt_inicio'] > $dt_inicio_loop) {
            ...
            if ($qry_atividade['dt_prev_fim'] >= $dt_ultimo_dia) {
                $width_div = $size_width * (diferenca_dt($qry_atividade['dt_inicio'], $dt_ultimo_dia) + 1);
            } else {
                $width_div = $size_width * (diferenca_dt($qry_atividade['dt_inicio'], $qry_atividade['dt_prev_fim']) + 1);
            }
        }
        ...
        //echo $link.monta_div_info($qry_atividade['numreg']);exit;
    }
}
...
for ($i = 0; $i < $dias; $i++) {
    ...
    //Criando a tabela temporaria
        if ($tipoBanco=="mysql") {
            ...
            query("CREATE TEMPORARY TABLE tmp_ativ (
				numreg VARCHAR(45),
				id_atividade varchar(20)default NULL,
				assunto varchar(500),
    ...
    while ($ar = farray($qry_ativ)) {
        ...
        if (numrows($qry_existe) == 0) {
            ...
            $sql = "INSERT INTO ".$prefixo_temp."tmp_ativ (numreg,id_atividade,assunto,id_pessoa,id_situacao,dt_inicio,dt_prev_fim,hr_inicio,hr_prev_fim,inicio,duracao,fim) VALUES('" . $ar['numreg'] . "','" . $ar['id_atividade'] . "','" . $ar['assunto'] . "','" . $ar['id_pessoa'] . "','" . $ar['id_situacao'] . "','" . $ar['dt_inicio'] . "','" . $ar['dt_prev_fim'] . "','" . $ar['hr_inicio'] . "','" . $ar['hr_prev_fim'] . "','" . $inicio_tmp . "','" . $duracao_tmp . "','" . $fim_tmp . "')";
            query($sql) or die(mysql_error() . "465");
        }
    }
    ...
    while ($ar_tmp = farray($qry_tmp)) {
        ...
        //echo $select_from."<br>";
        $qry_afetadas = query($select_from) or die(mysql_error());
...
if (substr($dt_inicio, 5, 2) == substr($dt_inicio_loop, 5, 2)) {
    $data_exibida = substr($dt_inicio, 8, 2) . ' - ' . substr($dt_inicio_loop, 8, 2) . ' de ' . $mes_name[intval(substr($dt_inicio_loop, 5, 2))] . ' de ' . substr($dt_inicio_loop, 0, 4);
} else {
    $data_exibida = substr($dt_inicio, 8, 2) . ' de ' . $mes_name[intval(substr($dt_inicio, 5, 2))] . ' de ' . substr($dt_inicio, 0, 4) . ' - ' . substr($dt_inicio_loop, 8, 2) . ' de ' . $mes_name[intval(substr($dt_inicio_loop, 5, 2))] . ' de ' . substr($dt_inicio_loop, 0, 4);
}
?>
...
<table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
    <tr>
        <td colspan="3" valign="center"><div align="left" valign="top"> &nbsp;&nbsp;&nbsp;<img src="images/seta.gif" width="4" height="7"/> <span class="tit_detalhes">Agenda Semanal </span>&nbsp;
                <input name="edtdt_inicio" type="text" id="edtdt_inicio"  value="<?php echo (!empty($_GET['dt_inicio'])) ? $_GET['dt_inicio'] : date("d/m/Y"); ?>"/>
                <select name="edtdias_semana" id="edtdias_semana">
                    <option value="5" <?php echo ($_GET['dias_semana'] == "5") ? 'selected="selected"' : ''; ?>>Seg-Sex</option>
                    <option value="7" <?php echo ($_GET['dias_semana'] == "7") ? 'selected="selected"' : ''; ?>>Dom-S&aacute;b</option>
                </select>
<?php
if ($ar_perfil['sn_bloquear_leitura'] == 'N') {
?>
    ...
    <?php
                ...
                if ($ar_perfil['sn_bloquear_leitura'] == 'N') {
                    ...
                    ?>
                &nbsp;
                <input name="button" type="button" class="botao_form" onclick="javascript:exibe_programa('agenda/calendario_agenda_semanal.php?dt_inicio=' + document.getElementById('edtdt_inicio').value + '&amp;dias_semana=' + document.getElementById('edtdias_semana').value<?php echo $comp_link; ?>);" value="Exibir" />
            </div></td>
        <td colspan="1" align="right"></td>
    </tr>
    <tr>
        <td colspan="3" valign="center" style="font-size:18px;"><img src="agenda/images/btn_back.png" width="15" height="15" style="cursor:pointer;" onclick="javascript:exibe_programa('agenda/calendario_agenda_semanal.php?dt_inicio=<?php echo dten2br(date("Y-m-d", strtotime($dt_inicio . " - 7 days"))); ?>&amp;dias_semana=<?php echo $dias; ?>&amp;usuario=<?php echo $id_usuario_logado; ?>');" /> <img src="agenda/images/btn_next.png" width="15" height="15" style="cursor:pointer;" onclick="javascript:exibe_programa('agenda/calendario_agenda_semanal.php?dt_inicio=<?php echo dten2br(date("Y-m-d", strtotime($dt_inicio . " + 7 days"))); ?>&amp;dias_semana=<?php echo $dias; ?>&amp;usuario=<?php echo $id_usuario_logado; ?>');" />
...
//Destruindo a tabela temporaria
                query("DROP TABLE ".$prefixo_temp."tmp_ativ");
?>