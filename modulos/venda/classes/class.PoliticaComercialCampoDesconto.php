<?php
/*
 * class.PoliticaComercialCampoDesconto.php
 * Autor: Alex
 * 08/12/2010 14:18
 * -
 *
...
class PoliticaComercialCampoDesconto{
    ...
    public function CalculaMaximoDescontoPermitido(){
        $ArrayCamposValores = array(
            ...
            'vl_tot_item'                   => $this->ArDadosVenda['vl_total_bruto']
        );
        $SqlPoliticaCampoDescontoItem = "SELECT ".((TipoBancoDados == 'mssql')?'TOP(1)':'')." numreg,pct_max_desc FROM is_param_politica_comercial_campo_desconto WHERE sn_ativo = 1 AND ('".date("Y-m-d")."' BETWEEN dthr_validade_ini AND dthr_validade_fim)";
